...
from frontend.utils.common import set_page_config, get_model_suffix, get_model_selection
from backend.config import MODEL_NAME, VM_IP, USERNAME, PASSWORD, LOCAL_PATH, FLAG_TESTINLOCAL, IMPORT_HFMODEL
...
# After document type selection and before file upload
if IMPORT_HFMODEL:
        ...
        if st.button("🔬 Iniciar Prueba"):
            with st.spinner(f"Procesando {doc_type}..."):
                try:
                    ...
                    # Execute test script with HF model if specified
                    cmd = ["python", "../backend/test.py", 
                          "--model", model_name,
                          "--input", "temp_test.xlsx"]
                    
                    if IMPORT_HFMODEL and 'hf_model' in st.session_state:
                        cmd.extend(["--hf_model", st.session_state['hf_model']])
